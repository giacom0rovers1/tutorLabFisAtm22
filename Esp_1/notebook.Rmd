...
colnames(BoUrbana_totRad) <- c('Init', 'DateTime', 'totRad')
BoUrbana_totRad <- select(BoUrbana_totRad, DateTime, totRad)
```
...
```{r}
HourlyData$DateTime <- force_tz(HourlyData$Date + HourlyData$Time, "Europe/Rome")

```
Sostituire qui `hourlyData` con `StationData` o `windData_filtered` o qualunque altro dataset di interesse per i confronti.
...
```{r}
combData <- inner_join(select(HourlyData, DateTime, totRad), BoUrbana_totRad, 
                       by = "DateTime",
...
I dati così formattati sono già pronti per uno **scatterplot**.

```{r}
ggplot(combData, aes(totRad_BoUrb, totRad_Davis)) + 
  geom_count() + 
  geom_abline(slope = 1, intercept = 0, alpha = 0.2) + 
  scale_size_area() +
  coord_fixed() +
...
```{r}
ggplot(tidyData, aes(DateTime, totRad, color = Stazione)) + 
  geom_line() + 
  theme_bw()
```
...
**1.** Seleziono due intervalli di pochi giorni, uno in estate e uno in inverno per valutare la presenza dell'ora legale, e scarico i dati di radiazione solare da dext3r per la stazione Bologna Urbana.
...
**2.** Controllo visivo: confronto le due serie temporali estive con un plot linea.
...
**3.** Controllo numerico: cerco la conferma stampando l'ora dei massimi.
...
**4.** Implemento la correzione tramite la funzione `force_tz()`.
```{r}
StationData$DateTime <- force_tz(StationData$Date + StationData$Time, "Europe/Rome")
```

**5.** Ricreo i due grafici:
```{r}
StationData %>% 
  select(DateTime, totRad) %>%
  filter(DateTime >= stSum,
...
**N.B.** Definire la time zone non come "CET" o "CEST" ma come **"Europe/Rome"** permette di gestire efficacemente il cambio tra ora solare e ora legale.
