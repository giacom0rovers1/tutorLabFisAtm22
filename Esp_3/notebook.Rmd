...
... e a *HSAF.raster*. Notare che il numero di layer ora è lo stesso.
...
*Vantaggi*: migliore gestione degli spazi, scala colore unica per tutti senza bisogno di imporlo.
*Manca*: gestione dei confini, la trasparenza dei valori NA.
...
Creo intanto una *tibble* di tutti i valori dei rasters aggregati. Ho perso le informazioni spaziali e temporali ma la corrispondenza 1:1 è conservata.
...
data %>% 
  ...
  scale_y_log10(limits = c(3, 40000)) + 
...
Sul modello di *statsFun*, viene costruita un'analoga funzione per gli **indicatori categorici**. In input non andranno i valori dei rasters ma delle maschere logiche costruite attorno a *THR*.
```{r}
