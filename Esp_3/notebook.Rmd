...
```{r}
plot(
  clamp(GPM.raster[["201610012100"]], 0.2, values=F), 
  main = names(GPM.raster[["201610012100"]]),
  col=c("blue","green","yellow","orange","red"),  
  xlab   = "Longitude (°E)",
  ylab   = "Latitude (°N)",
  panel.first = {
...
for(i in 1:length(RG.times)){
  RG.raster <- c(RG.raster, rasterize(cbind(latlon$lon, latlon$lat), MyGrid, values=RG.df[latlon$id,22]))
}
...
```{r}
hourlyMean <- function(IN.raster, IN.times, sel.times){
  
  stopifnot(nlyr(IN.raster)==length(IN.times))
  
  OUT.raster <- rast(MyGrid)
  OUT.times  <- character() 
  ...
  for(time in sel.times){
    ...
    OUT.times <- c(OUT.times, time)
  }
  
  names(OUT.raster) <- OUT.times
  stopifnot(nlyr(OUT.raster)==length(OUT.times))
...
```{r}
# seleziono i primi dieci giorni
...
```{r}
GPM.hourly <- hourlyMean(GPM.raster, GPM.times, sel.times)
GPM.final  <- mask(GPM.hourly, ItalyBG)
GPM.final

HSAF.hourly <- hourlyMean(HSAF.raster, HSAF.times, sel.times)
...
```{r}
par(mfrow=c(2,2))
plot(ERA5.final[["201610012100"]])
plot(RG.final[["201610012100"]])
plot(HSAF.final[["201610012057"]])
plot(GPM.final[["201610012100"]])
...
Per leggere bisogna usare poi: `readRaster()`



### **NEW** Funzione per plot mappe

### **NEW** Salvataggio immagini 2x2
