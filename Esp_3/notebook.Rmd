...
Più specifico del settore scientifico è il formato NetCDF (UNIDATA UCAR) [<https://www.unidata.ucar.edu/software/netcdf/>]. E\` un formato cosiddetto "self-explaining", nel senso che è predisposto per contenere informazioni ausiliarie che rendono l'interpretazione del file non ambigua (metadati). In questo caso uso la funzione `writeCDF()`.
...
Così come per `plot()`, anche la funzione base `hist()` ha un [metodo specifico]{.ul} per oggetti *SpatRaster*: il secondo argomento è il nome del livello selezionato:
...
Per aggregare tutti i livelli assieme, devo accedere direttamente ai valori delle celle. Posso farlo con la funzione `values()` che restituisce la [matrice dei dati del raster]{.ul} (n celle x n livelli):
...
**Nota tecnica**: gli accenti \`\` (tastiera US, oppure `AltGr + '`) si possono utiizzare per racchiudere un nome di variabile con caratteri non standard, tipo spazi, operazioni o parentesi (ad es. il nome della colonna "Precipitation (mm/h)" nella chiamata di `ggplot()`).
...
data %>% 
  ...
  geom_freqpoly(breaks = seq(0.2, 300, 2),
                #binwidth = function(x) 2 * IQR(x) / (length(x)^(1/3)), #binwidth = 0.5, 
                #boundary = 0.1) +  # , size = 1
  ) +
  scale_y_log10() +  # limits = c(1, 2000)
  # coord_trans(x = 'log10') +
  scale_x_log10(limits = c(0.2, 300)) + # scale_x_continuous(limits = c(0, 30)) +
  scale_color_jco() + 
  theme_bw()
...
data %>% 
  ...
  ggplot(aes(`Precipitation (mm/h)`, color = Product)) + 
  geom_freqpoly(binwidth = 0.5, boundaries = 0) +
...
statsFun_CAT <- function(Var, Ref){
  ...
  if(all(dim(xtab) == c(2,2))){
    hits <- as.double(xtab['TRUE' ,'TRUE' ])  #[2,2]#
    crej <- as.double(xtab['FALSE','FALSE'])  #[1,1]#
    ...
    return(data.frame(SMP, POD=NA, FAR=NA, BIAS=NA, ETS=NA, CSI=NA, ACC=NA, POFD=NA))
  }
