...
# = Measured Wind Speed and Wind Direction      v2022.10.12 = #
# =========================================================== #
...
if(Sys.info()["sysname"]=="Linux"){
  setwd("/home/giacom0rovers1/tutorLabFisAtm22/Old_2022/")  # Changed "Esp_1/" in "Old_2022/"
}else{
  setwd("C:/projects/tutorLabFisAtm22/Old_2022/")
} 


# Define the input filenames and assign them to a variable
StationDataFile <- "Dati Meteo - Stazione A"

# Read data as a table and assigning to the variable called 'StationData'
StationData  <- read.table(paste0(StationDataFile, ".txt"),
...
ExcTIME     <- StationData[which(StationData$Minutes > 0),]
StationData <- StationData[which(StationData$Minutes == 0),]
...
# Create a variable to storage the excluded values
ExcWSneg    <- StationData[which(StationData$wndSpeed < 0 | 
                                   StationData$wndSpeed >= 25 ),]
StationData <- StationData[which(StationData$wndSpeed >= 0 & 
                                   StationData$wndSpeed < 25 ),]
...
ExcWS3h_idx <- NULL     # Create a variable to store the ids of the excl. values
...
for(i in 3:dim(StationData)[1]){
  if((StationData[i,1]-StationData[i-2,1]) < 0.1    &    # 1h ~ 0.04 AbsTime
    ...
    ExcWS3h_idx <- c(ExcWS3h_idx,i-2,i-1,i)
  }
}

# Exclude Duplicates (i)
ExcWS3h_idx <- unique(ExcWS3h_idx)

# Exclude conditioned records from StationData
ExcWS3h     <- StationData[ExcWS3h_idx,]
StationData <- StationData[-ExcWS3h_idx,]
...
monPERC <- function(month) 100*length(which(StationData$"Month" == month))/
...
# NORTH WINDS LOOPING ----------------------------------------
for(ws in 2:6) {
  WindROSE[ii] <- length(which(
    ((StationData$"wndDir" >= 360-22.5)|(StationData$"wndDir" < 0+22.5))&
      (StationData$"wndSpeed" >= wsCAT[ws-1])&(StationData$"wndSpeed" < wsCAT[ws])))
  ii <- ii + 1 }
...
WindRosePLOT <- function(data) {barplot(data, 
                                        ...
                                        main = paste0("Station A (",
                                                      ifelse(length(unique(StationData$"Year")) == 1, unique(StationData$"Year"),
                                                             paste0(min(unique(StationData$"Year")),"-",max(unique(StationData$"Year"))) ),")"),
                                        sub = paste0("Calm ",round(100*length(which(StationData$"wndSpeed" < 2/3.6))/
...
fitLM <- lm(StatIND[,2]~StatIND[,1])
TextLM <- paste0("y = ",round(fitLM$coefficients[1],digits=2),
                 " + ",round(fitLM$coefficients[2],digits=2)," x")

# Save plot as PNG (also accepts PDF, JPG, etc...)
png(file=paste0(StationDataFile,"_ScatterPlot.png"))
plot(StatIND[,1], StatIND[,2], main = "Scatter Plot",
     xlab = "Wind Speed (m/s) - Station A", ylab = "Wind Speed (m/s) - Random Variation", pch = 20, col = "gray")
abline(lm(StatIND[,2]~StatIND[,1]), col = "red")
text(2,8, TextLM, col = "red")
dev.off()


# HOURLY MEAN WIND ============================================
WindHM <- function(hh){ c(
  length(which((((StationData$"wndDir"   >= (360-22.5)) | (StationData$"wndDir" <   (0+22.5))) &
                  (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh)))),
  length(which((StationData$"wndDir"   >=  (45-22.5)) & (StationData$"wndDir" <  (45+22.5)) &
                 (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndDir"   >=  (90-22.5)) & (StationData$"wndDir" <  (90+22.5)) &
                 (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndDir"   >= (135-22.5)) & (StationData$"wndDir" < (135+22.5)) &
                 (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndDir"   >= (180-22.5)) & (StationData$"wndDir" < (180+22.5)) &
                 (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndDir"   >= (225-22.5)) & (StationData$"wndDir" < (225+22.5)) &
                 (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndDir"   >= (270-22.5)) & (StationData$"wndDir" < (270+22.5)) &
                 (StationData$"wndSpeed" >=    (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndDir"   >= (315-22.5)) & (StationData$"wndDir" < (315+22.5)) &
                 (StationData$"wndSpeed" >=   (2/3.6)) & (StationData$"Hour" == hh))),
  length(which((StationData$"wndSpeed" <     (2/3.6)) & (StationData$"Hour" == hh)))
)
}
...
WindHM2  <- function(hh) mean(
  StationData[which((StationData$"wndSpeed" >= 0*2/3.6)&(StationData$"Hour" == hh)), 7]
)
iWindHM2 <- matrix(c(1:24),24,1)
for(h in 0:23) {iWindHM2[h+1,1] <- WindHM2(h)}

WindHM3  <- function(hh) sd(
  StationData[which((StationData$"wndSpeed" >= 0*2/3.6)&(StationData$"Hour" == hh)), 7]
)
